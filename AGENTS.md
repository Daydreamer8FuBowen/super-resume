...
SuperResume is a Codex plugin that helps users craft better resumes. It provides skills and hooks for resume writing, critique, formatting, and tailoring to specific job descriptions.

## Plugin Structure

```
.husky/           - Codex plugin manifest(s), if needed (toml/json)
...
1. Copy or symlink the plugin directory into Codex's plugin search path.
2. Type `/skill-name` in a Codex session to invoke it.
3. Iterate on the markdown directly.

If a plugin manifest is needed, create it at the repo root following the Codex plugin specification.
